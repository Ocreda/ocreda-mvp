...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { question_id, message, local_date }: ChatMessageRequest = await req
      .json();

    // 1. Load the original question
    const { data: question, error: qErr } = await supabase
      .from("questions")
      .select("id, question, relevant_note_ids")
      .eq("id", question_id)
      .eq("user_id", user.id)
      .maybeSingle();
    ...
    const systemPrompt =
      `You are the user's second brain — you only know what's in their source notes and user-authored messages below. Today's date (the user's local date) is ${local_date}; resolve any "today"/"tomorrow"/"yesterday"/weekday references against that.
...
- If nothing relevant exists, say so honestly`;
    ...
    // 5. Preserve user-authored context only. Generated assistant output never
    // becomes retrieval or reasoning context for a later response.
    ...
    return new Response(
      JSON.stringify({
        success: true,
        reply,
        relevant_notes: notesToUse.map((n) => ({
          id: n.id,
          summary: n.summary,
          raw_text: n.raw_text,
          connection_count: 0,
